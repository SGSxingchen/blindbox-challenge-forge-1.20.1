#!/usr/bin/env python3
"""生成并校验 P5 发行整改中的双语文案与 Forge 元数据。
...
license="License pending - no public redistribution"
...
displayName="盲盒挑战生存"
authors="盲盒挑战生存项目"
description='''服务端权威的盲盒挑战生存模组。P5 技术候选仅供受控验收，尚未取得公开再发行许可。'''
...
def render(relative: str) -> bytes:
    ...
    if relative == "pack.mcmeta":
        return (json.dumps({"pack": {"pack_format": 15, "description": "盲盒挑战生存 P5 技术候选资源（未公开发行）"}}, ensure_ascii=False, indent=2) + "\n").encode("utf-8")
    ...
    for line in MANIFEST.read_text(encoding="utf-8").splitlines():
        if line.startswith("|`"):
            ...
            if relative is not None:
                ...
                rows.append(f"|`{path}`|`{checksum}`|项目维护者（2026-08-07）|本地定义的中性双语与 Forge 元数据；不含人物、赛事、商标、画作、照片|随项目分发；仍需发行前项目许可证确认|2026-08-07|")
                updated.add(path)
                continue
        rows.append(line)
