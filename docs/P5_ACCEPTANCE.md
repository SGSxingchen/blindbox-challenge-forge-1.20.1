...
`multi-client.yml` 保持同一项“双客户端”必需工作流，但拆为两个必须同时成功的 job：`p1-p4-multi-client-regression` 只运行 P1—P4 原回归链；`p5-isolated-two-client` 使用新建专服世界、独立客户端目录和两名存活客户端，先执行三轮装饰方块的真实拾取→放置→双端观察→破坏→掉落→回收，再执行 P5 缓存压力。两者不共享世界、玩家状态、marker、交接平台或日志目录。单客户端 P5 也复用同一启动器的 `single` 模式。此结构变更后的新 SHA 必须重新取得六项门禁，旧 `d9c0aea` 仅保留为历史技术证据。
