...
全部 182 项正式资源均为当前项目的原创重绘或定义产物：68 张 PNG 为可复现重绘，其余模型、方块状态、战利品、双语与元数据由定义生成器闭合。原版图片只作需求输入，不读取、采样或混合，且不进入 Release。项目方提供素材与需求背景，许可本项目使用、修改与发行；该事实不外推为项目外第三方再授权。逐文件路径与 SHA-256 见 [资源审计清单](ASSET_MANIFEST.md)。
