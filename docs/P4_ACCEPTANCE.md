...
|任意门与安全落点|两门配对与反链、跨维安全落点；未加载、无落点、碰撞、损坏关联、自指及配对后出现第二安全点均拒绝；第二批已接入服务端配对/进入门体/两侧安全点和两门拆除/下方安全点不回跳/自指/未加载不强加载/碰撞拒绝探针，并新增双方安全点和持久失效回执逻辑。所有门请求必须先排队至 `ServerTick.END`，按 UUID、仍相交源门的碰撞盒和全量门事实二次校验后才传送，不能在触发门的旧位置包调用栈内换维；目标门免疫在真正迁移前预留且失败回滚。另接入同一次 `save-all flush → SIGKILL → 同世界重启、两真实客户端重连` 的跨维专项：杀前/后两门 UUID、双向反链、伙伴/安全点 `GlobalPos` 必须一致且不得重新配对；夹具先以不创建门户的 Forge 完整玩家迁移定位 Alice 主世界源门和 Bob 下界观察点并严格复验，随后仅 Alice 的真实前进键可经生产 `changeDimension` 门逻辑抵达下界门格；服务端必须等待原版 teleport id 客户端确认完成后，才严格核验 Alice 的位置与速度均为安全站立状态；Bob 必须同步观察远程 Alice、目标门和安全点；两客户端事实 marker 由服务端复验，旧 marker、超时或任一字段不符即失败，待 Hosted Runner|
