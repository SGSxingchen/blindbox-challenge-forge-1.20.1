...
`112b963` 的 artifact 已使八音盒范围缩小：真实普通 use、两端生产 S2C 和两端生产失败事件均有证据，仍无 PCM；缓存目录未被 artifact 上传，不能据此虚报缓存未写。下一轮只记录客户端最内层失败**异常类型**以定位下载/解码首错；不记录异常消息，故不泄露 URL、令牌、Cookie 或本地路径，也不回传服务器、不改变任何业务断言。P4 继续是 0/6。
