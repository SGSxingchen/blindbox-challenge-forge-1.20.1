...
顺序修复提交 `cbdd40b` 的双客户端运行 `31145551287` 又确认成功态的两份 marker、两名玩家位置和持久反链均通过，但仍未满足已有的“真实落地且零速度”门槛；该 artifact 未输出这两个值，故没有据此放宽条件或臆测改生产代码。本矩阵继续保持 0/6；下一轮只增加失败诊断字段来记录落地与速度，再按 Hosted Runner 首错处理。
