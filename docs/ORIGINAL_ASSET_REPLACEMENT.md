...
## 批次 C：中性双语与 Forge 元数据

`tools/generate_original_metadata.py` 以本地逐项定义的双语名称、服务端权威交互提示和固定 Forge 模板重建 `zh_cn.json`、`en_us.json`、`mods.toml` 与 `pack.mcmeta`。它保持 119 个双语键完全一致及所有格式参数，删除公开显示的人员、赛事和缩写标记；同时明确当前为 P5 技术候选、没有公开再发行许可。该工具同样提供 `--check` 和 `--update-manifest`，质量门禁会校验键闭合、非空值、元数据状态和逐字节防漂移。

批次 C 后，当前工作树 182 项正式资源都可追溯到项目维护者的本地生成/定义来源；这**不是**项目整体许可证。`b864e83` 已将三批资源整改交给 Hosted Runner，并与质量、专服、强杀恢复、真实单客户端、真实双客户端和汇总同 SHA 通过。随后仍须由权利人确认许可证、可再发行范围和署名条件，并完成名称/商标审查，才允许 Release。
