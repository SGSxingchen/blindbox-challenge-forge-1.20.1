...
|期次|目标|范围|验收|
|---|---|---|---|
|P0：已冻结|锁定已定稿边界|保持台账、资源边界和可配置项；不处理 TAC/魔女服/乐魂|文档一致，默认奖池为空|
...
|P5：正式资产与发布审查|自制美术/音频边界|原创像素转绘、模型/数据/双语定义、URL 音频缓存压力测试|`e33a3fe` 已取得独立 P5 双端拓扑的同 SHA 六门禁；真实双端覆盖 GUI→S2C→PCM、LRU 驱逐重下、单飞和损坏缓存重试。全部 182 项正式资源为当前项目的原创重绘或定义产物；原版图片不进入 Release。最终候选仍须重跑同 SHA 门禁、生成 Jar/SHA-256 与发布件|
...
核心模组不引入 TAC、魔女服装模组或乐魂外部内容，也不制作对应物品。参考图可作为原创像素化/风格化转绘方向，但不得直接裁切照片作为贴图。在线音乐不随包分发，按本文件的 URL 广播与客户端安全缓存方案处理；发行包不含原版图片；第三方 URL 的可用性、版权和长期稳定性不作承诺。
