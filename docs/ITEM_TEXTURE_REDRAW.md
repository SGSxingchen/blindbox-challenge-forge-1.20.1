...
2026-08-11 本地结果：工具单测 107 项全部通过；59 张正式 PNG、生成器与资源清单三方 SHA-256 全部一致；资源清单 182 条路径全部存在且哈希一致；贴图、模型与元数据三个生成器的完整 `--check` 均通过；质量静态契约通过；Temurin Java 17.0.17 下 `clean check` 成功。为恢复完整生成器契约，在不修改正式资源字节的前提下，另闭合了 8 张方块 PNG、95 项权威模型/方块状态/战利品 JSON 与 4 项权威元数据。
...
元数据在安装前还会执行最低结构校验：双语文件必须是非空字符串映射且键集合与权威载荷一致，`pack.mcmeta` 必须具备有效的资源包格式与说明，`mods.toml` 必须包含当前模组以及 Forge、Minecraft 的完整必要依赖字段。结构错误统一以中文 `ValueError` 拒绝，不会触碰正式资源。
