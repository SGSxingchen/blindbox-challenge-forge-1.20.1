...
**公开发行仍被真实法律条件阻塞：**P5 批次 A 已将 68 张历史 PNG 替换为可复现的原创像素贴图；[正式资源审计清单](docs/ASSET_MANIFEST.md) 现余 99 项等待权利人/法务确认，83 项仍须确认项目发行许可证。技术门禁全绿不构成授权；在收到可再发行书面授权，或完成全部可审计原创替换及相应名称/商标审查前，Issue #5 必须保持 OPEN，不得创建 tag、公开 Release 或发布 Jar。TAC、魔女服、乐魂、原始照片与压缩素材继续排除。原始资料在 `source-package/`，不得修改或删除；原压缩包备份在 `archive/`。
